...
:root{
  ...
  --ok:#7fd18d; --ok-soft:#12291a; --on-ok:#0c2413;
  --focus:#f0a92c;
...
html{color-scheme:dark}   /* a wall screen in a cellar: one controlled theme, DESIGN-SYSTEM.md §1 */
...
.filters button{box-shadow:var(--elev-1);font:600 var(--text-small)/1 var(--font-body);
...
td{padding:var(--space-5) var(--space-4) var(--space-5) 0;border-bottom:2px solid var(--line);
   vertical-align:middle}
tr.row:focus-within td{background:var(--surface)}
td svg{height:128px;width:auto;color:var(--ink);display:block}
.pub{font-weight:600}
