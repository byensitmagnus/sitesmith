...
runId: screen-02-atelier-printworks-impeccable
...
## 0. Settled product truth (from frozen pack only)

- **Subject:** Atelier Møn Printworks — coastal letterpress studio (posters, map editions, shop bags).
...
| Cultural home | Danish island print culture: ferry terminals, harbour boards, small-press prospectuses, gallery wall cards, map sheets. |
...
2. **Small-press prospectus leaflet** — commission sequence as numbered pamphlet spreads. *Why:* limited-run commerce ritual.
3. **Edition colophon / proof plate** — paper, size, press as primary UI grammar. *Why:* facts earn belief.
...
| Source | **api** (`https://impeccable.style/api/roll`) |
...
| Index formula | `3 + floor(u32be(sha256("direction:index:"+key)) / 0xffffffff * (7-2))` → hash `b206ddca…` → unit ≈ 0.6954 → index **6** |
| Compositions | API returned 3; skill default omits composition deal unless `IMPECCABLE_COMPOSITIONS=1` — not used as co-decision here |
...
FIRST VIEWPORT: Dark board hero with three named rows + evidence press-sheet for focused row; Enquire in destination column and top rail.
FORM: Grounded #6 Ferry departure board; seed key h2h-atelier-2026-07-31; split-flap grammar absorbed; kiosk-print held as alternate.
```
